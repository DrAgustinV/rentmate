...
serve(async (req) => {
  ...
  try {
    ...
    const signatureData = {
      ...
      manager_embed_slug: docusealData.find((s: { role: string; slug?: string }) => s.role === 'Manager')?.slug || null,
      tenant_embed_slug: docusealData.find((s: { role: string; slug?: string }) => s.role === 'Tenant')?.slug || null,
